...
def enhancedFeatureExtractorDigit(datum):
    ...
    extra_features = [1, 0, 0]
    first placeholder = 1 if there is one white region; 0 otherwise
    second placeholder = 1 if there are two separate white regions; 0 otherwise
    third placeholder = 1 if there are three separate white regions; 0 otherwise
    ...
    "*** YOUR CODE HERE ***"
    marked_pixels = np.array(features)
    extra_features = np.zeros(3, int)
    counter = 0

    for x in range(DIGIT_DATUM_WIDTH):
        for y in range(DIGIT_DATUM_HEIGHT):
            if marked_pixels[DIGIT_DATUM_HEIGHT*x+y] == 0:
                counter += 1
                dfs(x, y, marked_pixels)
...
def dfs(x, y, marked_pixels):
  
    if x+1 < DIGIT_DATUM_WIDTH: 
        if marked_pixels[DIGIT_DATUM_HEIGHT*(x+1)+y] == 0:
            marked_pixels[DIGIT_DATUM_HEIGHT*(x+1)+y] = 1
            dfs(x+1, y, marked_pixels)
    ...
    if y+1 < DIGIT_DATUM_HEIGHT:
        if marked_pixels[DIGIT_DATUM_HEIGHT*x+(y+1)] == 0:
            marked_pixels[DIGIT_DATUM_HEIGHT*x+(y+1)] = 1
            dfs(x, y+1, marked_pixels) 
...
def readCommand( argv ):
    ...
    parser.add_option('-c', '--classifier', help=default('The type of classifier'),
                      choices=['mostFrequent', 'nb', 'naiveBayes', 'perceptron', 'binaryLinear', 'multiclassLinear', 'oneVsRestLinear'], default='mostFrequent')
    ...
    elif(options.classifier == "binaryLinear"):
        assert len(legalLabels)==2, "Classifier type 'binaryLinear' requires number of legal labels to be exactly two"
        classifier = linearLearning.BinaryLinearClassifier(legalLabels,options.iterations)
    elif(options.classifier == "multiclassLinear"):
        classifier = linearLearning.MulticlassLinearClassifier(legalLabels,options.iterations)
    elif(options.classifier == "oneVsRestLinear"):
        classifier = linearLearning.OneVsRestLinearClassifier(legalLabels,options.iterations)
