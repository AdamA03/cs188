...
def der_sigmoid_dx(x):
    ...
    """
    "*** YOUR CODE HERE ***"
    return sigmoid(x) * (math.exp(-x) * sigmoid(x))
...
def softmax(x):
    ...
    """
    "*** YOUR CODE HERE ***"
    #this is the array of the nd array 
    e = np.exp(x)
    all_softmax = 0
...
class LinearRegression:
    ...
    def trainAnalytical(self, trainingData_x, trainingData_y):
        ...
        "*** YOUR CODE HERE ***"
        weights = np.dot( np.linalg.inv( np.dot(trainingData_x.T, trainingData_x) ), (np.dot(trainingData_x.T, trainingData_y)) )
    ...
    def der_loss_dw(self, x, y_true, weights):
        ...
        """
        "*** YOUR CODE HERE ***"
        return der_quadLoss_dx(self.hypothesis(x),y_true)* der_dot_dw(x, weights)
...
class BinaryLinearClassifier:
    ...
    def loss(self, x, y_true):
        ...
        """
        "*** YOUR CODE HERE ***"
        # return - (y_true - self.hypothesis(x)) * (y_true - self.hypothesis(x))
        return quadLoss(self.hypothesis(x), y_true)
    ...
    def der_loss_dw(self, x, y_true, weights):
        ...
        return der_quadLoss_dx(self.hypothesis(x), y_true) * der_sigmoid_dx(np.dot(x, weights)) * der_dot_dw(x, weights)
...
class MulticlassLinearClassifier:
    ...
    def hypothesis(self, x):
        ...
        for i in range(numClasses):
            labels.append(np.dot(x, self.weights[i]))
