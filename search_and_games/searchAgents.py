...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self): ##not the full Pacman state?
        """
        ...
        state = {}
        state['position'] = self.startingPosition
        state['pelletsEaten'] = [False, False, False, False]
        return state

    def goalTest(self, state): #1 goal state
        """
        Returns whether this search state is a goal state of the problem.
        """
        "*** YOUR CODE HERE ***"
        
        return state['pelletsEaten'] == [True, True, True, True]
    ...
    def getResult(self, state, action):
        ...
        successor['pelletsEaten'] = list(state['pelletsEaten'])
        if (nextx, nexty) in self.corners:
            index = self.corners.index((nextx, nexty))
            if state['pelletsEaten'][index] == False:
                successor['pelletsEaten'][index] = True    
        return successor
...
def cornersHeuristic(state, problem):
    ...
    def manhattanHeuristic(xy1, xy2):  
        """ This is our helper function (we simply modified the manhattanHeuristic function given above) """   
        return abs(xy1[0] - xy2[0]) + abs(xy1[1] - xy2[1])
    
    #self.corners = ((1,1), (1,top), (right, 1), (right, top)) *explanation (for project check-in)*
    foodHeuristics = []
    if state["pelletsEaten"] == [True, True, True, True]:
            return 0
    else:
        for i in range(4):
             if state["pelletsEaten"][i] == False:
                    foodHeuristics.append(manhattanHeuristic(state["position"], corners[i]))
    return max(foodHeuristics)
...
def foodHeuristic(state, problem):
    ...
    for foodPos in foodGrid.asList():
        remainingPellets.append(mazeDistance(position, foodPos, problem.startingGameState))
        
    return max(remainingPellets) 
