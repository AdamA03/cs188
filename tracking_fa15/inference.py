...
class ExactInference(InferenceModule):
    ...
    def update(self, observation, gameState):
        ...
        for ghost in self.allPositions:
            new_evidence = self.getObservationProb(observation, gameState.getPacmanPosition(), ghost, self.getJailPosition())
            self.beliefs[ghost] *= new_evidence
            self.beliefs.normalize()
    ...
    def predict(self, gameState):
        ...
        for oldPos in self.allPositions:
            ...
            for newPos, p in newPosDist.items():
                newWeight[newPos] += self.beliefs[oldPos] * p
        self.beliefs = newWeight
...
class ParticleFilter(InferenceModule):
    ...
    def update(self, observation, gameState):
        ...
        weighted_distribution = DiscreteDistribution()
        pacmanPosition = gameState.getPacmanPosition()
        
        jailPosition = self.getJailPosition()
        
        for ghostPosition in self.particles:
            evidence = self.getObservationProb(observation, pacmanPosition, ghostPosition, jailPosition)
            weighted_distribution[ghostPosition] += evidence
        size = len(self.particles)
        self.particles = []
        if weighted_distribution.total() == 0:
            self.initializeUniformly(gameState)
        else:
            while size > 0:
                particle = weighted_distribution.sample()
    ...
    def predict(self, gameState):
        ...
        for oldPos in self.particles:
            ...
            new_location.append(newPosition)

        self.particles = new_location
    ...
    def update(self, observation, gameState):
        ...
        for ghostPosition in self.particles:
            ...
            for i in range(self.numGhosts):
                ...
                evidence *= self.getObservationProb(observation[i], pacmanPosition, ghostPosition[i], jailPosition)
            weighted_distribution[ghostPosition] += evidence
        size = len(self.particles)
        self.particles = []
        if weighted_distribution.total() == 0:
